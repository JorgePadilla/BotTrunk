...
- [ ] Fund it on TestNet: ~~ALGO from the dispenser~~ (10 ALGO received Sept 10 from https://bank.testnet.algorand.network), then opt in to TestNet USDC (ASA `10458941`) in Defly and get test USDC from https://faucet.circle.com (network: Algorand Testnet; the faucet has a per-address cooldown of ~2 h and a transfer to a non-opted-in account bounces).
