...
**Proving a group without spending:** `POST /v2/transactions/simulate` with `{"allow-empty-signatures": true}` evaluates a group against real MainNet state and submits nothing. It settled the onboarding question on Sept 12: `[0.2 ALGO → agent][agent opts in][USDC → agent]` as one group **succeeds**; the same three with the USDC before the opt-in **fails** with `must optin, asset 31566704 missing from …`, which is the node's own string, the one the facilitator forwards verbatim (`Payments::NodeErrors` now translates it). Watch the encoding: algod's canonical JSON wants address fields (`snd`, `rcv`, `close`, `asnd`, `arcv`, `aclose`, `sgnr`) as **base32**, every other byte field as base64 — the Python SDK's `dictify()` hands back raw bytes for both. Simulation proves a group is *valid*; it does not exercise the facilitator co-signing a fee-payer group, which still needs a real call.
