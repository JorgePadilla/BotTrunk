...
The catalog is fetched at startup and re-read when it is more than five minutes old (on `tools/list`, on `bottrunk_catalog`, and before refusing a `bottrunk_*` tool it has not heard of), so a long-lived server picks up new services without a restart; when the set of tool names changes it sends `notifications/tools/list_changed`. If the catalog cannot be fetched at startup the server starts **degraded** rather than exiting: the two free tools remain, `bottrunk_catalog` returns the unreachable URL and what to do, and paid tools appear on the next successful refresh. A refresh that fails keeps the last good list — a blip must not delete an agent's tools mid-session. Paid calls send `User-Agent: bottrunk-mcp/<version>` so `/admin/stats` can tell them from any other node process.
...
BotTrunk's own services are `Fulfillers::*`, all subclasses of `Fulfillers::Base`, which owns the SSRF guard (a host resolving to private space is refused before any request), the Faraday client, and the Result shapes: a 4xx is `Result.success` with that status (answered, never settled), a genuine upstream failure is `Result.failure`. Today: `ScrapeMarkdown`, `PageMetadata`, `ExtractLinks`, `UrlHealth`, `DomainDns`, `EmailCheck` (all code, no dependencies beyond Nokogiri and stdlib Resolv/OpenSSL); `ScrapeMarkdownJs`, `ScreenshotUrl` and `PdfUrl`, which share `RenderedCapture` and reach a browser through `Rendering::Cloudflare` rather than running Chromium in this container (a render is 300-500 MB and an OOM here would take the deposit endpoints down with it); `DepositBac` and `RfqGlobal` (human-fulfilled, §7). The two human services share `OrderLifecycle`: `DepositOrder` carries a bank, an account and an amount; `WorkOrder` carries a brief and the result of the work. `Gateway::HandlePaidCall` only ever calls `order.update!(status:)` and reads `order.token`, so a new kind of order joins the paid loop without touching the payment path, and `OrderLookup` is what lets one token resolve to either. `Fulfillers::HumanJob` then serves *every* service where a person does the work: it reads what the buyer must supply, how long they are promised and how many jobs can be in flight from the catalog entry's `job:` spec, so `rfq-global`, `verify-business-hn`, `translate-es-en` and `human-review` share one class and adding the next is a catalog entry with no new code. `LocalPrice` is the other half of the same idea from the opposite direction: field work recorded once on a weekly round and sold many times through `prices-hn`, with every reading carrying the day it was observed and how it was learned. The two capture services answer base64 inside JSON, because the 402 advertises `application/json` and the Bazaar schema describes an object; a render over 1.5 MB is refused with 422 rather than returned, and a refusal is never settled.
